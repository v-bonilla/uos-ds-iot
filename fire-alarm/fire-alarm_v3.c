...
{  
    ...
    while(1) {
        ...
        if(FIRE == 0 && current_temp >= 6400 && current_light >= 120) {
            process_start(&notification, NULL);
            FIRE = 1;
        }
        else if(FIRE == 1 && (current_temp < 6400 || current_light < 120)) {
            process_exit(&notification);
            leds_off(LEDS_RED); // In case it exits with the led on
            FIRE = 0;
        }
...
{  
    ...
    while(1) {
        ...
        PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&notification_timer));
        ...
        // leds_blink();
        leds_on(LEDS_RED);
        etimer_set(&led_timer, CLOCK_CONF_SECOND / 2);
